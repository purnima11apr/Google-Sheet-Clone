...
const clipboardIcon = document.getElementById("paste");
...
function toggleButtonsStyle(button, isSelected) {
  //this function is calling every button in highlightOptionButtonsOnFocus function
  if (isSelected) {
...
function highlightOptionButtonsOnFocus() {
  //this function is for to know if cell is have to highlight button or not
...
function onClickBold(boldButton) {
  //this will triggered when user clicks on boldbutton
...
function onClickItalic(italicButton) {
  //this will triggered when user clicks on Italic button
  /**
...
function highlightTextAlignButtons(textAlignValue) {
  //this function will triggered after user select one button from text align
  ...
  for (let i = 0; i < textAlignElements.length; i++) {
    if (textAlignElements[i].getAttribute("data-value") === textAlignValue) {
      //getattribute function will give us value of data of slected
...
function onClickTextAlign(textAlignButton) {
  //this will triggered when user clicks on TEXT ALIGN button
...
//FOR CUT ICON
function cutTextFromCell() {
  if (activeCell) {
    const textToCut = activeCell.textContent;
    copyTextToClipboard(textToCut); // Copy the text to clipboard
    activeCell.textContent = ""; // Clear the cell
  }
}
